<script lang="ts">
  ...
  import { buildCSRFHeaders, Toast, api, clickOutside } from '../Components/helper';
  ...
  async function handleDeleteProject() {
    ...
    const result = await api(() => axios.delete(`/projects/${project.id}`, { headers: buildCSRFHeaders() }), { showSuccessToast: false });
...
<AppLayout title={project.name} {nav_workspaces} {nav_projects_standalone} {unread_count} activeProjectId={project.id} chat_workspace_id={chatWorkspaceId} chat_workspace_name={chatWorkspaceName}>
  <div class="px-5 sm:px-8 lg:px-10 py-6 space-y-5" in:fly={{ y: 20, duration: 500 }}>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-white/[0.04] ring-1 ring-stone-900/5 dark:ring-white/10 rounded-2xl p-5">
      ...
      <div class="flex flex-wrap items-center gap-2 shrink-0">
        <!-- Batch dropdown — filter, not an action -->
        <select
        ...
        <!-- Version dropdown — owner only, groups Minor/Major into one control -->
        {#if user.id === project.owner_id}
          <div class="relative">
            <button
              onclick={() => { showVersionMenu = !showVersionMenu; showKebabMenu = false; }}
              disabled={isBumping}
            ...
            >
              ...
              Version
              <svg class="w-3 h-3 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
            </button>
            {#if showVersionMenu}
              ...
              <div
                use:clickOutside
                onclickoutside={() => showVersionMenu = false}
                class="absolute right-0 top-full mt-1.5 w-44 bg-white dark:bg-surface-dark ring-1 ring-stone-900/10 dark:ring-white/10 rounded-xl shadow-lg p-1.5 z-30"
                transition:fade={{ duration: 100 }}
              >
                <button
                  onclick={() => { showVersionMenu = false; bumpVersion(false); }}
                  disabled={isBumping}
                  class="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-stone-700 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-white/[0.06] rounded-lg transition-colors disabled:opacity-50 cursor-pointer text-left"
                >
                  <svg class="w-3.5 h-3.5 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 11l-5-5-5 5"/></svg>
                  <span class="flex flex-col">
                    <span>Minor bump</span>
                    <span class="text-[10px] font-normal text-stone-400 dark:text-stone-500">x.N.x — fitur baru</span>
                  </span>
                </button>
                <button
                  onclick={() => { showVersionMenu = false; bumpVersion(true); }}
                  disabled={isBumping}
                  class="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-stone-700 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-white/[0.06] rounded-lg transition-colors disabled:opacity-50 cursor-pointer text-left"
                >
                  <svg class="w-3.5 h-3.5 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 11l-5-5-5 5M12 18V6"/></svg>
                  <span class="flex flex-col">
                    <span>Major bump</span>
                    <span class="text-[10px] font-normal text-stone-400 dark:text-stone-500">N.x.x — breaking change</span>
                  </span>
                </button>
              </div>
            {/if}
        ...
        <!-- Kebab menu — secondary actions (Activity, Delete) -->
        <div class="relative">
          <button
            onclick={() => { showKebabMenu = !showKebabMenu; showVersionMenu = false; }}
            class="inline-flex items-center justify-center w-9 h-9 bg-white dark:bg-white/5 ring-1 ring-stone-900/10 dark:ring-white/10 hover:ring-stone-900/20 dark:hover:ring-white/20 text-stone-600 dark:text-stone-300 rounded-xl transition-all duration-300 active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/60 cursor-pointer"
            title="More actions"
          ...
          {#if showKebabMenu}
            ...
            <div
              use:clickOutside
              onclickoutside={() => showKebabMenu = false}
              class="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-surface-dark ring-1 ring-stone-900/10 dark:ring-white/10 rounded-xl shadow-lg p-1.5 z-30"
              transition:fade={{ duration: 100 }}
            >
              <button
                onclick={() => { showKebabMenu = false; showActivity = true; }}
                class="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-stone-700 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-white/[0.06] rounded-lg transition-colors cursor-pointer text-left"
              >
                <svg class="w-3.5 h-3.5 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                Activity
              </button>
              {#if user.id === project.owner_id}
                <div class="h-px bg-stone-100 dark:bg-white/[0.06] my-1"></div>
                <button
                  onclick={() => { showKebabMenu = false; showDeleteProject = true; }}
                  disabled={isDeletingProject}
                  class="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors disabled:opacity-50 cursor-pointer text-left"
                >
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"/></svg>
                  Hapus project
                </button>
              {/if}
            </div>
  ...
  {#if showDeleteProject}
    <DeleteConfirmModal
      ...
      warningText="Semua task, batch, dan data terkait project ini akan dihapus permanen."
