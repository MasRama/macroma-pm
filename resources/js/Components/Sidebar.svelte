...
<aside data-testid="sidebar" class="fixed left-0 top-0 h-screen w-[260px] bg-white dark:bg-surface-dark backdrop-blur-xl border-r border-slate-200 dark:border-white/[0.06] flex flex-col z-40 transition-all duration-300 overflow-hidden">
  ...
  <nav class="flex-1 overflow-y-auto px-3 py-4 custom-scrollbar relative z-10">
    <p class="text-xs uppercase tracking-widest text-slate-400 dark:text-slate-600 px-3 mb-3 font-semibold">PROJECTS</p>
    
    <div class="space-y-0.5">
      {#if projects.length > 0}
        {#each projects as project, index}
          <a use:inertia href="/projects/{project.id}" class="group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all {activeProjectId === project.id ? 'bg-primary-50 dark:bg-primary-500/15 text-primary-600 dark:text-primary-400 border-l-2 border-primary-500' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/[0.04] border-l-2 border-transparent'}">
            <span class="w-2 h-2 rounded-full {dotColors[index % 4]} shrink-0"></span>
            <span class="truncate">{project.name}</span>
          </a>
        {/each}
      {:else}
        <p class="px-3 py-2 text-xs italic text-slate-400 dark:text-slate-600">No projects yet</p>
      {/if}
    </div>
  </nav>
