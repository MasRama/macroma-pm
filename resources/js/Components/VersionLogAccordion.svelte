...
<div data-testid="version-log-accordion" class="mt-3 border-t border-white/5 pt-3">
  <button onclick={toggle} class="flex items-center gap-2 text-xs text-slate-400 hover:text-slate-200 transition-colors w-full text-left">
  ...
  {#if isOpen}
    <div transition:slide={{ duration: 200 }} class="mt-3 space-y-3">
      ...
      {:else}
        {#each logs as log}
          <div class="flex flex-col gap-1 bg-white/5 p-2 rounded-lg">
            <div class="flex items-center gap-2">
              <span class="font-mono text-[10px] bg-slate-700/50 text-slate-300 px-1.5 py-0.5 rounded">{log.version}</span>
              <span class="text-xs text-slate-300">
                {log.column_from ? (columnNames[log.column_from] || log.column_from) : 'Created'} &rarr; {columnNames[log.column_to] || log.column_to}
              </span>
            </div>
            {#if log.note}
              <div class="text-xs text-slate-400 mt-0.5">{log.note}</div>
            {/if}
